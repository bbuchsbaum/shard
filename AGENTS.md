...
This project uses **beads** (`bd`) for git-backed issue tracking and **gastown** (`gt`) for multi-agent orchestration.
...
Gastown coordinates multiple Claude Code agents working simultaneously with persistent context through git-backed storage.
...
**Worker Roles** (perform project work):
- **Crew**: Persistent workers with human control; ideal for exploratory work
- **Polecat**: Ephemeral workers managed by Witness; best for discrete tasks
...
Agents execute assigned work immediately without waiting for confirmation. This makes Gas Town a coordinated, responsive system.
...
7. **Hand off** - Provide context for next session (use `gt handoff` if in gastown)
